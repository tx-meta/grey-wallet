...
export class SupabaseSignUpUseCase {
  constructor(
  ...
  async execute(request: SupabaseSignUpRequest): Promise<SupabaseSignUpResult> {
    try {
      // 1. Validate input
      ...
      // 6. Prepare response
      const responseData: SupabaseSignUpResponse = {
        user: {
          id: supabaseUser.id,
  ...
  private validateSignUpRequest(request: SupabaseSignUpRequest): void {
